...
//import es.upm.fi.dia.oeg.morph.base.parser._
  ...
  def createRunner(connection:Connection, properties:MorphProperties):MorphBaseRunner = {
    ...
    if(morphProperties.queryFilePath.isDefined) {
      runner.sparqlQuery = try {
        ...
        // Mi codigo nuevo
        val bufferedSource = scala.io.Source.fromFile(queryFile);
        val lines= bufferedSource.mkString;//.getLines().mkString
        bufferedSource.close;
        val indexSkyline = lines.toUpperCase().indexOfSlice("skyline of".toUpperCase());
        val (new_lines,skyline_line) =  
            if (indexSkyline > - 1) (lines.slice(0, indexSkyline - 1), lines.substring(indexSkyline));
       
        logger.info("NL: " + new_lines.toString());
        logger.info("Sky: " + skyline_line.toString());
        //val query = QueryFactorySkyline.read(queryFile);
        val query = if (indexSkyline > - 1) {
          runner.skylineClause = Some(skyline_line.toString());
          QueryFactory.create(new_lines.toString());
          //QueryFactorySkyline.read(queryFile);
          } 
        else {QueryFactory.read(queryFile);}
        
        val op:Op = SSE.parseOp("(bgp (?s ?p ?o))") ;
        
        //val op:opSkyline = SSE.parseOp(skyline_line) ;
        //val op:opSkyline = opSkyline(skyline_line);
        /*Op op = SSE.parseOp(skyline_line);
        System.out.println(op);
        */
        // Linea comentada temporalmente
