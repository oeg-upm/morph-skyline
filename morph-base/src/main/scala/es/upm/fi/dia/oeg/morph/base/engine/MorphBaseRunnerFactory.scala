...
//import com.exasol.jdbc._

import es.upm.fi.dia.oeg.morph.base.parser.QueryFactorySkyline
  ...
  def createRunner(connection:Connection, properties:MorphProperties):MorphBaseRunner = {
    ...
    if(morphProperties.queryFilePath.isDefined) {
      ...
      //runner.sparqlQuery = try {
        ...
        runner.sparqlQuery = Some(query.getQuery)
        Some(query);
        //Some(query.getQuery);
      } catch {
        case e:Exception => {
