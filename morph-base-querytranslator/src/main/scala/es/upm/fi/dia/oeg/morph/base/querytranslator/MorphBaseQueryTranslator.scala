...
//	var mapping:Map[Node, List[ZSelectItem]]= Map.empty;
  ...
  def getSelectItemsByNode(node:Node , oldSelectItems:List[ZSelectItem] ) : Set[ZSelectItem] = {
    val nameSelectVar = nameGenerator.generateName(node);

    val result = oldSelectItems.flatMap(oldSelectItem => {
      val oldAlias = oldSelectItem.getAlias();
      val selectItemName = {
        if(oldAlias == null || oldAlias.equals("")) {
          oldSelectItem.getColumn();
  ...
  def transExtend(opExtend:OpExtend ) : IQuery = {
    ...
    val subOpSelectItems = subOpSQL.getSelectItems();
    val subOpSelectItemsList = subOpSelectItems.toList;
  ...
  def transGroup(opGroup:OpGroup ) : IQuery = {
    ...
    val transOpGroup:SQLQuery = if(subOpSQL.isInstanceOf[SQLQuery]) {
      subOpSQL.asInstanceOf[SQLQuery];
    } else if(subOpSQL.isInstanceOf[SQLUnion]) {
      val sqlQuery = new SQLQuery();
      sqlQuery.addLogicalTable(subOpSQL);
      sqlQuery;
    } else {
      throw new Exception("error while translating OpGroup!");
    }
    transOpGroup.setDatabaseType(dbType);

    subOpSQL.generateAlias();
    ...
    for(exprAggregator <- aggregators) {
      ...
      if(subOpSQL.isInstanceOf[SQLQuery]) {
  ...
  def translate(op:Op) : IQuery  = {
    ...
    val inferredTypesString = this.mapInferredTypes.mkString("\n");
    logger.info(s"\nthis.mapInferredTypes = ${inferredTypesString}");
  ...
  override def translate(sparqlQuery:Query ) : IQuery  = {
    ...
    mapping = this.betaGenerator.mapping
    result
  }
  ...
  def translateFromQueryFile(queryFilePath:String ) : IQuery  = {
    ...
    val lineas = scala.io.Source.fromFile (queryFilePath);
